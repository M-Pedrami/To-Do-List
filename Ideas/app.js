...
const creatTask = () =>{
// creating an array of colors and a random number generator for the div.task background. the array can be easily extended without having to change any other part of the code. ❗❗❗❗ first I created the following two variables in the global scope. It lead to a problem that a random color would have selected once and then used for every other newTask generated within the creatTask function. I had to move it to the function scope for it to work as it was intended to ❗❗❗❗. ⚠⚠⚠⚠ MUST use let to assign randomIndex because we want to be abale to reassign its value below(look here:👀) when we press the pallette button. Remember pressing the palette button happens in the same👈❗ iteration as the newTask div is created so if we use const to assign a value, the value cannot be changed within the same iteration. the reason it changes for a new newTask div is that by click the add input button we fire another👈❗ iteration of the creatTask function.
